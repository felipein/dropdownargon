...
@Component({
  selector: 'app-tusd-icms',
  templateUrl: './tusd-icms.component.html',
  styleUrls: ['./tusd-icms.component.scss']
})
export class TusdIcmsComponent implements OnInit {
  ...
  windowHandle:Window;
  portalConcluido:boolean=false;

  //login 13.155.259/0001-53
  //senha financeiro@nutrilar.com
  ...
  relatorio:RelatorioRemocaoTUSDBaseCalculoICMS = new RelatorioRemocaoTUSDBaseCalculoICMS();
  ...
  constructor(private autenticacaoService:AutenticacaoService,
    ...
    private calculoTusdIcms:CalculoTusdIcmsService){}
  ...
  watchEquatorial(handler: Window){
    setTimeout(() => {
      console.log('equatorial timeout');
      if(handler.localStorage != null && handler.localStorage.length > 20){

        this.portalConcluido=true;
        console.log('portal concluido');
        let mensagemStorage:LocalStorageEquatorial = new LocalStorageEquatorial(handler.localStorage.getItem('conta_ativa') || '',
        handler.localStorage.getItem('jwt') || '',
        handler.localStorage.getItem('nome') || '');
        mensagemStorage.contasContrato = JSON.parse(handler.localStorage.getItem('contasContrato') || '');
        ...
        .then(importacaoFatura=>{
          ...
          console.log(importacaoFatura as ImportacaoFatura);
      ...
      if(!this.portalConcluido)
        this.watchEquatorial(handler);
    }, 1000);
  }
  ...
  portalEquatorial(){
    ...
    //this.windowHandle.localStorage.clear();
    this.watchEquatorial(this.windowHandle);
  }
  ...
  gerarRelatorio(){
    ...
    this.calculoTusdIcms.calculoTusdIcms(this.faturas)
    .then(result=>{

      this.relatorio = result as RelatorioRemocaoTUSDBaseCalculoICMS;
