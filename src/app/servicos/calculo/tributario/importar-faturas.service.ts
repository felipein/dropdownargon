...
export class ImportarFaturasService {
  ...
  importarFaturas(importacaoFatura:ImportacaoFatura){
  ...
  private async apiCall(importacaoFatura:ImportacaoFatura){
    ...
    const data = await this.httpClient.post(environment.api as string + 'api/ImportarFaturas',importacaoFatura, {'headers':headers}).toPromise();
    return data;
  }
}
